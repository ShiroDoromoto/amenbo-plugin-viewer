# amenbo-plugin-viewer

PC から離れているあいだも、スマートフォンから [**Amenbo**](https://github.com/ShiroDoromoto/amenbo)
のタスクの進み具合を見る。**Amenbo Viewer** の実装。

Amenbo 本体は外部と通信しない。運営もデータをホスティングしない。**置き場は利用者が持つもの**で、
そこへレコードを片方向に運ぶ。**運ぶのは動いたぶんだけ**で、スマートフォンは自分が読んだ点から先を取る。

## 何が入っているか

| 部品 | 動く場所 |
|---|---|
| プラグイン | 利用者の PC（Windows / macOS / Linux） |
| Worker | 利用者の Cloudflare |
| iOS アプリ | iPhone |
| Android アプリ | Android 端末 |

4つは同じ契約（レコードの形・暗号の封筒・QR に載せるもの・エンドポイントの仕様）を共有する。
**契約が変わると4つとも変わる**ので、1つのリポジトリに置いてある。

## 構成

部品ごとにディレクトリを分ける。それぞれ別のツールチェーンを持つので、リポジトリ直下は
どの部品にも渡さない。

| | 中身 |
|---|---|
| [`plugin/`](plugin/) | プラグイン（Go） |
| [`worker/`](worker/) | Cloudflare Worker |
| [`app/`](app/) | スマートフォンアプリ（Flutter。`ios/` と `android/` はこの下に生成される） |
| [`guards/`](guards/) | どの部品にも属さない検査（ツリーの形と、部品をまたぐ綴りの規約を見る。`make guards` と CI が回す） |

**契約を散文で置く場所は作らない。** 置けば、どの部品を直しても文書の追従が要り、
その漏れは静かに起きる——3つとも動いていて文書だけが嘘、という形になる。
契約は動くものの側に持たせてある: 経路は Worker のルート表、レコードの形は送る側と取り込む側の
コード、封筒は両側のコードと、両方を通しで踏む検証。**ずれは読んで見つけるのではなく、通して見つける。**

ビルドはルートの `Makefile` が束ねるだけで、実体は各部品の `Makefile` が持つ。

```
make plugin   # その部品だけ（worker / app も同じ）
make build    # 3部品まとめて
```

## 経路は1つ

| 経路 | 利用者の手数 |
|---|---|
| 利用者自身の Cloudflare Worker ＋ D1 | Cloudflare のアカウント1つ。**API トークンを1回貼るだけ**で、立てるのはプラグイン |

一方通行で、運ぶ単位は**レコード1件ずつ**。

**必ず封をする。** 置き先は利用者が借りている場所なので、暗号文しか置かない。鍵はネットワークを
通らず、QR でスマートフォンへ渡る。

**経路は2つあった。** もう1つは mac のアプリ専用 iCloud コンテナで、設定ゼロで動く道だった。
落とした理由は**差分カーソルと前提が噛み合わないこと**——iCloud は受領を返さず、届く順も保証しない。
実際に初回配置で 44% が一度も上がらないまま、送信は成功を返した。詳細は
[`plugin/README.md`](plugin/README.md)「経路は2つあった」。

置き場が D1 なのは、**1キーの上書きでは答えられないことが2つある**ため——
「この順番から先で何が変わったか」と「この行を忘れろ」。だからスマートフォンが読むのは範囲で、
PC が送るのは動いた行だけになる。

## ドキュメント

**各部品の README しか無い。** 全体に効く話も、その話が効く部品の README が持つ。
設計を散文で置く場所を作らないのは契約と同じ理由で、実装が進むほど文書だけが古くなり、
動いているものと食い違ったまま残るからである。決めた理由は、それが効くコードの隣に書く。

## 名前

| | 名前 |
|---|---|
| リポジトリ | `amenbo-plugin-viewer` |
| アプリ名（ストア表示） | **Amenbo Viewer** |
| バンドル ID / パッケージ名 | `work.amenbo.viewer` |

大小は、その文字列が**実体か呼び名か**で決まる。機械がその綴りのまま解決するもの——打つコマンド、PATH に載る名前、リポジトリ名、URL、バンドル ID——は小文字の `amenbo`。文章の中で製品を指すときは `Amenbo` / `Amenbo Viewer`。迷ったら「`Amenbo` へ書き換えたら何かが動かなくなるか」を問う。なるなら小文字側で、そこは `guards/check-program-name-casing.sh` が見張っている。

## 状態

開発中。まだ動くものは配布していない。

## ライセンス

Apache-2.0。[LICENSE](LICENSE) を参照。
