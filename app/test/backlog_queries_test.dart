...
void main() {
  ...
  group('the one list face', () {
    ...
    test(
      ...
      () {
        ...
        expect(
          store.tasks(TaskQuery(changedSince: since)).map((row) => row.id),
          [4],
        );
      },
    );
  ...
  group('what moved since the last visit', () {
    const lastLook = '2026-08-09T00:00:00Z';
    const after = '2026-08-09T09:00:00Z';

    test('closing, filing and commenting are counted apart', () {
      store.applyPage([
        BacklogChange.put(
          'task',
          1,
        ...
        BacklogChange.put(
          'task',
          2,
          task(id: 2, status: 'rejected', completedAt: after),
        ),
        BacklogChange.put('task', 3, task(id: 3, createdAt: after)),
        BacklogChange.put('task', 4, task(id: 4)),
        BacklogChange.put(
          'task_comment',
          1,
          comment(id: 1, taskId: 4, createdAt: after),
        ),
      ]);

      final moved = store.sinceLastLook(lastLook);
      expect(moved[Moved.finished]!.value, 2);
      // The one filed after the mark, and not the two that were merely closed after it.
      expect(moved[Moved.filed]!.value, 1);
      expect(moved[Moved.commented]!.value, 1);
    });

    test('what happened before the mark is not what moved since', () {
      store.applyPage([
        BacklogChange.put(
          'task',
          1,
          task(id: 1, status: 'done', completedAt: lastLook),
        ),
        BacklogChange.put(
          'task_comment',
          1,
          comment(id: 1, taskId: 1, createdAt: lastLook),
        ),
      ]);

      expect(
        store.sinceLastLook(lastLook).values.map((count) => count.value),
        everyElement(0),
      );
    });
    ...
    test('it counts only the project being looked at', () {
      store.applyPage([
        ...
        BacklogChange.put(
          'task_comment',
          1,
          comment(id: 1, taskId: 2, createdAt: after),
        ),
      ]);

      expect(store.sinceLastLook(after, projectId: 16)[Moved.filed]!.value, 0);
      expect(
        store.sinceLastLook(lastLook, projectId: 16)[Moved.filed]!.value,
        1,
      );
      expect(
        store.sinceLastLook(lastLook, projectId: 16)[Moved.commented]!.value,
        0,
      );
      expect(
        store.sinceLastLook(lastLook, projectId: 20)[Moved.commented]!.value,
        1,
      );
    });

    test('an archived project is out of it, as it is out of the bundles', () {
      store.applyPage([
        BacklogChange.put('project', 16, project(id: 16, archived: 1)),
        BacklogChange.put('task', 1, task(id: 1, createdAt: after)),
        BacklogChange.put(
          'task_comment',
          1,
          comment(id: 1, taskId: 1, createdAt: after),
        ),
      ]);
    ...
    test('a comment that arrived ahead of its task still counts', () {
      store.applyPage([
        BacklogChange.put(
          'task_comment',
          1,
          comment(id: 1, taskId: 99, createdAt: after),
        ),
      ]);

      expect(store.sinceLastLook(lastLook)[Moved.commented]!.value, 1);
    });
  });
